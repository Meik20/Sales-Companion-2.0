...
export function Hero() {
  ...
  const searchPhrases = lang === 'en' ? searchPhrasesEn : searchPhrasesFr

  const results = [
    {
      name: 'Tech Yaoundé SARL',
      sector: lang === 'en' ? 'Technology' : 'Technologie',
      city: 'Yaoundé'
    },
    {
      name: 'BTP Douala Group',
      sector: lang === 'en' ? 'Construction & BTP' : 'BTP & Construction',
      city: 'Douala'
    },
    {
      name: 'Agro Littoral SA',
      sector: lang === 'en' ? 'Agribusiness' : 'Agroalimentaire',
      city: 'Douala'
    }
  ]
  ...
  }, [displayText, isDeleting, phraseIndex, searchPhrases])
      ...
      <div className="mx-auto grid max-w-6xl items-center gap-12 px-5 lg:grid-cols-[1.05fr_0.95fr]">
        <div className="flex flex-col items-start">
          <span className="inline-flex items-center gap-2 rounded-full border border-border bg-card px-3.5 py-1 text-xs font-medium text-muted-foreground shadow-sm hover:border-primary/40 transition-colors">
            ...
            {lang === 'en' ? 'B2B Intelligence Cameroon' : 'Intelligence B2B Cameroun'}
          </span>
          ...
          <h1 className="mt-5 font-heading text-4xl font-semibold leading-[1.08] tracking-tight text-foreground text-balance sm:text-5xl lg:text-6xl">
            {lang === 'en' ? (
              <>
                The database of Cameroonian <span className="text-primary">companies</span>
              </>
            ) : (
              <>
          ...
          <p className="mt-5 max-w-xl text-lg leading-relaxed text-muted-foreground text-pretty">
            {lang === 'en' ? (
              <>
                Sales Companion <span className="text-[#1B7A3E] font-semibold">2.0</span> centralizes Cameroon&apos;s B2B company directory, your CRM pipeline, and team management into one unified tool, accessible anywhere — even offline.
              </>
            ) : (
              <>
                Sales Companion <span className="text-[#1B7A3E] font-semibold">2.0</span> centralise l&apos;annuaire des entreprises camerounaises, votre pipeline CRM et la gestion d&apos;équipe dans un seul outil, accessible partout — même hors-ligne.
              </>
            )}
          </p>
          ...
          <dl className="mt-10 grid w-full max-w-md grid-cols-3 gap-6 border-t border-border pt-6">
            {[
              { value: '500K+', label: lang === 'en' ? 'indexed companies' : 'entreprises indexées' },
              { value: '10', label: lang === 'en' ? 'regions covered' : 'régions couvertes' },
              { value: 'PWA', label: lang === 'en' ? '100% offline' : '100% hors-ligne' }
            ].map((stat) => (
        ...
        <div className="relative">
          <div className="rounded-2xl border border-border bg-card p-2.5 shadow-xl shadow-primary/5 transition-transform hover:shadow-primary/10">
            ...
            <div className="rounded-xl bg-background p-4 border border-border/50">
              ...
              <p className="mt-4 mb-2 text-[11px] font-medium uppercase tracking-wide text-muted-foreground flex items-center gap-1.5">
                ...
                {lang === 'en' ? '47 verified companies found' : '47 entreprises vérifiées trouvées'}
              </p>
              ...
              <div className="mt-4 flex items-center gap-2 overflow-hidden pt-1">
                {(lang === 'en' ? ['Prospecting', 'Negotiation', 'Won'] : ['Prospection', 'Négociation', 'Gagné']).map((stage, i) => (
                  <span
                    key={stage}
          ...
          <div className="absolute -bottom-5 -left-4 hidden max-w-[240px] rounded-xl border border-primary/30 bg-card/95 p-3.5 shadow-xl backdrop-blur-md sm:block animate-float">
            <p className="text-[11px] font-semibold text-primary flex items-center gap-1.5">
              ...
              {lang === 'en' ? 'AI Sales Companion' : 'Companion IA Pro'}
            </p>
            <p className="mt-1 text-xs leading-snug text-muted-foreground italic">
              {lang === 'en'
                ? '“Draft an introductory email tailored to the tech SME in Yaounde”'
