...
export function AuthGuard({ children }: PropsWithChildren) {
  ...
  useEffect(() => {
    ...
    const isPending = (user as { emailVerificationPending?: boolean } | null)?.emailVerificationPending

    if (firebaseUser.emailVerified && isPending) {
      setFinalizing(true)
      firebaseUser.getIdToken(true).then(async (token) => {
        try {
  ...
  if (loading) {
    return (
      <div style={{
        ...
        flexDirection: 'column', gap: 16, color: colors.textMid,
      }}>
        <span style={{
          width: 32, height: 32,
      ...
      <div style={{
        ...
        <div style={{
          ...
          <h1 style={{
            ...
            fontFamily: "'Syne',sans-serif", margin: '0 0 10px',
          }}>
            Vérification email requise
          </h1>
          <p style={{ fontSize: 14, color: colors.textMid, lineHeight: 1.7, margin: '0 0 24px' }}>
            Votre compte a été créé mais votre email<br />
            <strong style={{ color: colors.text }}>{user.email}</strong><br />
          ...
          <div style={{
            ...
            fontSize: 13, color: colors.textMid, lineHeight: 1.7,
