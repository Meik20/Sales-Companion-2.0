export const fr = {
  landing: {
    features: 'Fonctionnalités',
    roles: 'Profils',
    pricing: 'Tarifs',
    testimonials: 'Témoignages',
    login: 'Connexion',
    startFree: 'Commencer gratuitement',
    heroBadge: 'Intelligence B2B Cameroun',
    heroTitle1: 'Base de données des',
    heroTitle2: 'entreprises',
    heroTitle3: 'Camerounaises',
    heroTitle4: 'Prospectez plus vite,',
    heroTitle5: 'vendez mieux',
    heroSub:
      "Sales Companion 2.0 est l'annuaire B2B de référence qui centralise votre base d'entreprises camerounaises, votre pipeline CRM et la gestion d'équipe en un seul outil accessible partout.",
    freeToStart: 'Gratuit au démarrage',
    discoverFeatures: 'Découvrir les fonctionnalités',
    installApp: "Installer l'app",
    noCardRequired: 'Aucune carte requise',
    officialDatabase: 'Base camerounaise officielle',
    pwaBanner: {
      qrAria: "Code QR pour installer l'application",
      scanToInstall: 'Scannez pour installer',
      title: 'Installer sur votre téléphone',
      desc: 'Téléchargez Sales Companion 2.0 directement sur votre téléphone pour un accès rapide et un fonctionnement hors ligne. Aucun compte App Store requis.',
      installAria: "Installer l'application",
      installBtn: 'Installer maintenant',
      startBtn: 'Commencer'
    },
    stats: {
      indexed: 'entreprises indexées',
      regions: 'régions du Cameroun',
      sectors: 'secteurs couverts',
      offline: 'mode hors-ligne PWA'
    },
    featuresSection: {
      title1: 'Outils de',
      title2: 'prospection B2B',
      title3: 'au Cameroun',
      sub: 'Optimisez vos ventes avec Sales Companion 2.0 : base de données officielle, CRM léger et intelligence commerciale pour le terrain camerounais.',
      f1Title: 'Annuaire des entreprises',
      f1Desc:
        'Accédez aux coordonnées de 50 000+ entreprises camerounaises. Filtrez par secteur, région et ville pour cibler vos prospects.',
      f1Tag: '→ Données RCCM & NIU',
      f2Title: 'Pipeline Commercial CRM',
      f2Desc:
        "Gérez votre cycle de vente de A à Z. Suivez l'avancement de vos négociations et ne manquez aucune relance client.",
      f2Tag: '→ Suivi des ventes',
      f3Title: "Gestion d'Équipe Commerciale",
      f3Desc:
        'Pilotez vos commerciaux en temps réel. Assignez des prospects et analysez les performances de votre force de vente.',
      f3Tag: '→ Dashboard Manager',
      f4Title: 'Intelligence Artificielle Sales',
      f4Desc:
        "Utilisez l'IA pour rédiger vos emails d'approche et analyser les besoins de vos prospects dans le contexte camerounais.",
      f4Tag: '→ Companion IA',
      f5Title: 'Segments de Recherche',
      f5Desc:
        "Sauvegardez vos critères pour retrouver instantanément vos listes d'entreprises par secteur ou zone géographique.",
      f5Tag: '→ Filtres sauvegardés',
      f6Title: 'Application Mobile PWA',
      f6Desc:
        "Installez l'annuaire sur votre smartphone. Accédez à vos prospects même sans connexion internet lors de vos tournées.",
      f6Tag: '→ Mode hors-ligne'
    },
    rolesSection: {
      title1: 'Solutions pour',
      title2: 'tous les',
      title3: 'commerciaux',
      sub: "Que vous soyez un agent indépendant ou un directeur commercial à Douala ou Yaoundé, notre outil s'adapte à vos besoins.",
      indepTitle: 'Commercial Indépendant',
      indepDesc:
        'Gagnez en efficacité dans votre prospection quotidienne. Accédez à la meilleure base de données B2B du pays.',
      indepPerk1: 'Recherche entreprises illimitée',
      indepPerk2: 'Gestion de pipeline personnelle',
      indepPerk3: 'Sauvegarde de prospects',
      indepPerk4: 'Aide à la vente par IA',
      mgrTitle: 'Directeur Commercial / Manager',
      mgrDesc:
        'Déployez une stratégie commerciale data-driven. Supervisez vos équipes et augmentez votre taux de conversion.',
      mgrPerk1: 'Vue globale du pipeline équipe',
      mgrPerk2: 'Distribution des leads',
      mgrPerk3: 'Rapports de performance',
      mgrPerk4: 'Contrôle des accès'
    },
    plansSection: {
      title1: 'Tarifs de',
      title2: 'votre abonnement',
      title3: 'Sales Companion 2.0',
      sub: 'Choisissez le plan adapté à votre volume de prospection. Tous nos tarifs sont conçus pour le marché camerounais.',
      free: 'Plan Gratuit',
      freeBadge: 'Toujours disponible',
      freePrice: '0',
      freePeriod: 'Pour toujours',
      pro: 'Pro',
      proBadge: 'Recommandé',
      proPrice: '15 000',
      proPeriod: 'par mois',
      starter: 'Starter',
      starterBadge: 'Économique',
      starterPrice: '5 000',
      starterPeriod: 'par mois',
      enterprise: 'Enterprise',
      enterpriseBadge: 'Équipes',
      enterprisePrice: '50 000',
      enterprisePeriod: 'par mois',
      pFree1: '10 recherches par mois',
      pFree2: 'Accès base entreprises',
      pFree3: 'Pipeline personnel',
      pFree4: 'Application PWA installable',
      pStarter1: '10 recherches par jour',
      pStarter2: 'Filtres avancés',
      pStarter3: 'Export Excel',
      pStarter4: 'Pipeline personnel',
      pPro1: '20 recherches par jour',
      pPro2: 'Companion IA commercial',
      pPro3: 'Pipeline illimité',
      pPro4: 'Recherches sauvegardées',
      pPro5: 'Export Excel',
      pPro6: 'Support prioritaire',
      pEnterprise1: '50 recherches par jour',
      pEnterprise2: 'Tout le plan Pro inclus',
      pEnterprise3: 'Gestion équipe & rôles',
      pEnterprise4: 'Assignation de prospects',
      pEnterprise5: 'Dashboard manager temps réel',
      pEnterprise6: 'Support dédié',
      startFreeBtn: 'Commencer gratuitement',
      chooseStarterBtn: 'Choisir Starter',
      chooseProBtn: 'Choisir Pro',
      contactEnterpriseBtn: 'Choisir Enterprise',
      recommended: 'Recommandé'
    },
    testiSection: {
      title1: 'Ce que disent',
      title2: 'nos',
      title3: 'utilisateurs',
      t1Text:
        '"En quelques minutes, j\'ai trouvé 30 entreprises BTP à Douala avec numéros et dirigeants. Avant, je passais des heures à compiler ces infos manuellement."',
      t1Role: 'Commercial BTP — Douala',
      t2Text:
        '"Le mode manager change tout. Je vois le pipeline de toute mon équipe en temps réel et j\'assigne les prospects directement depuis mon téléphone."',
      t2Role: 'Directrice commerciale — Yaoundé',
      t3Text:
        "\"L'app fonctionne même en zone avec signal faible. Pour les tournées terrain dans les régions, c'est indispensable. La PWA s'installe comme une vraie app.\"",
      t3Role: 'Commercial terrain — Bafoussam'
    },
    useCasesSection: {
      title1: "Ils l'utilisent",
      title2: 'pour',
      title3: 'accélérer leurs ventes',
      sub: 'Découvrez comment nos utilisateurs transforment leur prospection commerciale au quotidien avec Sales Companion 2.0.',
      uc1Title: 'BTP & Construction',
      uc1Desc:
        "Identifiez les nouvelles entreprises de BTP, trouvez les promoteurs immobiliers et fournisseurs de matériaux. Exportez les contacts pour vos appels d'offres en un clic.",
      uc2Title: 'Tech & Services B2B',
      uc2Desc:
        "Ciblez les PME technologiques ou les cabinets de conseil. Utilisez l'assistant IA pour personnaliser votre approche et qualifier vos leads avant même le premier appel.",
      uc3Title: 'Banque & Assurance',
      uc3Desc:
        'Trouvez les entreprises par tranches de capital pour proposer vos services financiers (crédits, assurances professionnelles). Gérez vos relances via le pipeline intégré.'
    },
    faqSection: {
      title1: 'Foire Aux',
      title2: 'Questions',
      title3: '(FAQ)',
      sub: 'Tout ce que vous devez savoir sur Sales Companion 2.0.',
      q1: "D'où proviennent les données des entreprises ?",
      a1: 'Nos données sont agrégées à partir de sources officielles publiques (RCCM, NIU) et sont mises à jour régulièrement pour garantir la meilleure fiabilité possible sur le marché camerounais.',
      q2: "Puis-je utiliser l'application sans connexion Internet ?",
      a2: 'Absolument. En tant que Progressive Web App (PWA), vous pouvez installer Sales Companion 2.0 sur votre téléphone et consulter vos données sauvegardées même sans connexion. La synchronisation se fait automatiquement au retour du réseau.',
      q3: 'Combien de commerciaux puis-je ajouter à mon équipe ?',
      a3: "Les plans Pro et Entreprise vous permettent d'ajouter des commerciaux à votre équipe. Le plan PRO vous permet d'ajouter jusqu'à 3 membres actifs, tandis que le plan ENTREPRISE vous permet d'en ajouter jusqu'à 10. Vous bénéficiez d'un tableau de bord centralisé pour piloter l'ensemble de leur activité.",
      q4: "Est-il possible d'exporter mes prospects ?",
      a4: "Oui, vous pouvez exporter l'ensemble de votre pipeline et de vos recherches au format Excel ou CSV (fonctionnalité disponible sur les plans Pro et Entreprise) pour les intégrer à vos autres outils."
    },
    privacy: {
      label: 'Transparence',
      title: 'Vos données, ',
      titleHighlight: 'votre contrôle',
      subtitle:
        'Nous traitons vos informations avec rigueur et respect. Voici exactement ce que nous collectons, pourquoi, et comment vous gardez le contrôle total.',
      collected: {
        title: 'Données collectées',
        item1: 'Nom, adresse e-mail et mot de passe (inscription)',
        item2: 'Informations de profil professionnel (secteur, région)',
        item3: 'Historique de recherche et filtres utilisés',
        item4: 'Prospects ajoutés à votre pipeline commercial',
        item5: "Journaux de connexion et données d'utilisation"
      },
      purposes: {
        title: 'Finalités du traitement',
        item1: 'Fournir et améliorer nos services de prospection B2B',
        item2: 'Personnaliser votre expérience (suggestions de secteurs)',
        item3: 'Gérer votre compte, abonnement et facturation',
        item4: 'Vous contacter en cas de support ou mises à jour importantes',
        item5: 'Assurer la sécurité et prévenir les utilisations abusives'
      },
      rights: {
        title: 'Vos droits',
        item1: 'Accéder à toutes vos données personnelles à tout moment',
        item2: 'Corriger ou mettre à jour vos informations depuis votre profil',
        item3: 'Demander la suppression complète de votre compte',
        item4: 'Exporter vos prospects et recherches sauvegardées (CSV)',
        item5: 'Révoquer votre consentement sans justification'
      },
      security: {
        title: 'Nos engagements de sécurité',
        tls: {
          title: 'Chiffrement TLS',
          desc: 'Communications chiffrées en transit (HTTPS/TLS 1.3)'
        },
        firebase: {
          title: 'Firebase Secure',
          desc: 'Données hébergées sur Google Firebase avec auth sécurisée'
        },
        resale: {
          title: 'Zéro revente',
          desc: 'Vos données ne sont jamais vendues ni partagées avec des tiers'
        },
        hosting: {
          title: 'Hébergement certifié',
          desc: 'Infrastructure dans des data centers certifiés ISO 27001'
        },
        contactText:
          'Pour toute demande relative à vos données personnelles, contactez-nous via le support intégré accessible après connexion. Nous répondons dans un délai de 72 heures.'
      }
    },
    ctaSection: {
      title1: 'Prêt à prospecter',
      title2: 'plus',
      title3: 'intelligemment',
      title4: '?',
      sub: 'Rejoignez les commerciaux camerounais qui utilisent Sales Companion 2.0 pour trouver, suivre et conclure plus vite.',
      createAccount: 'Créer un compte gratuit',
      login: 'Se connecter',
      note: 'Aucune carte bancaire requise · Gratuit au démarrage'
    },
    footer: {
      desc: 'La plateforme B2B dédiée aux commerciaux et managers camerounais. Intelligence terrain, PWA hors-ligne, base de données officielle.',
      madeIn: 'Made in Cameroun 🇨🇲',
      product: 'Produit',
      app: 'Application',
      support: 'Support',
      search: 'Recherche',
      pipeline: 'Pipeline',
      assistance: 'Assistance',
      account: 'Mon compte',
      rights: 'Tous droits réservés.'
    }
  },
  sidebar: {
    quickFilters: 'Filtres rapides',
    region: 'Région',
    allRegions: 'Toutes les régions',
    city: 'Ville',
    allCities: 'Toutes les villes',
    sector: 'Secteur',
    allSectors: 'Tous les secteurs',
    search: 'Rechercher',
    aroundMe: 'Autour de moi',
    radius: 'Rayon',
    prospection: 'Prospection',
    searchProspects: 'Recherche prospects',
    pipeline: 'Pipeline commercial',
    savedSearches: 'Recherches sauvegardées',
    support: 'Support',
    help: 'Aide & Support',
    systemStatus: 'Statut Système',
    systemOnline: 'En ligne',
    systemOnlineDesc: 'Données synchronisées en temps réel',
    systemOffline: 'Hors ligne',
    systemOfflineDesc: 'Cache local actif · Données sur cet appareil',
    systemSimulated: 'Mode simulé',
    reconnect: 'Reconnecter',
    simulate: 'Simuler',
    offlineMode: 'Mode hors ligne',
    team: 'Équipe',
    teamManagement: "Gestion d'équipe",
    account: 'Compte',
    settings: 'Paramètres',
    theme: 'Mode',
    light: 'Clair',
    dark: 'Sombre',
    admin: 'Administration',
    dashboard: 'Dashboard',
    users: 'Utilisateurs',
    companies: 'Entreprises',
    imports: 'Imports',
    activity: 'Activité',
    config: 'Config',
    payments: 'Paiements',
    logout: 'Déconnexion',
    logoutSuccess: 'Déconnexion réussie',
    logoutError: 'Erreur de déconnexion',
    user: 'Utilisateur',
    member: 'Membre',
    manager: 'Manager',
    independent: 'Indépendant',
    adminRole: '🔴 Admin',
    managerRole: '🟡 Manager',
    independentRole: '🟢 Indépendant',
    memberRole: '🔵 Membre',
    supportRole: '🎧 Support',
    supportAgentRole: '🎧 Agent Support',
    crmSection: '🎧 Espace Relation Client',
    crmClients: 'Mes Clients CRM',
    crm: 'CRM Clients',
    detecting: '⏳ Détection…',
    saved: 'Sauvegardés'
  },
  header: {
    planFree: 'Plan Gratuit',
    myProfile: 'Mon Profil',
    settings: 'Paramètres',
    logout: 'Déconnexion',
    login: 'Connexion',
    search: 'Recherche',
    profile: 'Profil',
    aiAssistant: 'Companion IA'
  },
  search: {
    title: "Recherche d'entreprises",
    placeholder: 'Entreprise, secteur, ville…',
    quickAll: 'Tous',
    btp: 'BTP',
    commerce: 'Commerce',
    tech: 'Tech',
    sante: 'Santé',
    advancedFilters: 'Filtres avancés',
    region: '📍 Région',
    allRegions: 'Toutes les régions',
    city: '🏙 Ville',
    allCities: 'Toutes les villes',
    geolocation: '📡 Géolocalisation',
    aroundMe: '📍 Autour de moi',
    aroundMeDone: '✅ Autour de moi',
    detecting: '⏳ Détection…',
    sector: "🏢 Secteur d'activité",
    allSectors: 'Tous les secteurs',
    reset: '✕ Réinitialiser',
    clearAll: '✕ Tout effacer',
    apply: 'Appliquer',
    locationDenied: 'Localisation refusée. Autorisez-la dans votre navigateur.',
    locationUnavailable: 'Position indisponible.',
    locationNotSupported: 'Géolocalisation non supportée.',
    results: 'Résultats',
    companiesFound: 'entreprise(s) trouvée(s)',
    quotaExceeded: 'Quota de recherche épuisé. Vos crédits seront réinitialisés.',
    searchError: "Impossible d'exécuter la recherche. Veuillez réessayer.",
    noResult: 'Aucun résultat',
    noResultDesc: "Essayez d'élargir vos critères de recherche.",
    clearFilters: 'Effacer les filtres',
    resetSearch: 'Réinitialiser la recherche',
    findIdealProspects: 'Trouvez vos prospects idéaux',
    findIdealProspectsDesc:
      'Recherchez par nom, secteur, ville ou utilisez les filtres rapides ci-dessous pour démarrer.',
    startFirstSearch: 'Lancez votre première recherche →',
    findProspectsCta: 'Trouver des prospects →',
    searchingProspects: 'Recherche en cours…',
    activeOpportunities: 'opportunités actives',
    filterByStage: 'Filtré par :',
    allStages: 'Tous les stades',
    clearStageFilter: 'Effacer le filtre',
    noProspectInStage: 'Aucun prospect dans ce stade.',
    showAllProspects: 'Afficher tous les prospects',
    myPipelineFull: '→ Voir mon pipeline complet',
    aiAssistant: 'Companion IA',
    aiGreeting:
      "Bonjour 👋 Je suis votre Companion IA.\nPosez-moi vos questions sur la prospection au Cameroun, les secteurs d'activité, ou demandez-moi de rédiger un email d'approche.",
    aiThinking: '⏳ Réflexion en cours…',
    aiPlaceholder: "Ex: Comment aborder un DG dans l'Agroalimentaire ? (Entrée pour envoyer)",
    loadingSearch: 'Chargement de la recherche...',
    inPipeline: '✓ Dans le pipeline',
    adding: '⏳ Ajout…',
    retry: '↺ Réessayer',
    addPipeline: '+ Pipeline',
    save: 'Enregistrer',
    saved: 'Enregistré',
    alreadySaved: '✓ Déjà enregistrée',
    saving: '⏳ Sauvegarde…',
    saveSearch: 'Sauvegarder',
    savedSearch: 'Sauvegardée',
    defaultSearchLabel: 'Recherche',
    call: 'Appeler',
    whatsapp: 'WhatsApp',
    email: 'Email',
    pitchAi: 'Script IA',
    companySavedToast: '✓ Entreprise enregistrée',
    companySavedDesc: 'Ajoutée à vos favoris avec succès.',
    prospectAddedToast: '✓ Prospect ajouté au pipeline',
    prospectAddedDesc: 'Ajouté à votre pipeline avec succès.',
    saveSearchSuccess: 'Recherche sauvegardée',
    saveSearchError: 'Sauvegarde impossible',
    companies: 'entreprises',
    updatedAt: 'Mis à jour',
    btpDouala: 'BTP Douala',
    newCompanies: 'Nouvelles entreprises',
    techYaounde: 'Tech Yaoundé',
    startupsPme: 'Startups & PME',
    agro: 'Agroalimentaire',
    allCameroon: 'Tout Cameroun',
    transport: 'Transport',
    commercialPipeline: 'Pipeline commercial',
    prospection: 'Prospection',
    negotiation: 'Négociation',
    conclusion: 'Conclusion'
  },
  feedback: {
    loading: 'Chargement…',
    pleaseWait: 'Veuillez patienter.',
    errorOccurred: 'Une erreur est survenue'
  },
  pipeline: {
    title: 'Pipeline',
    subtitle: "Suivi de la prospection jusqu'à la conclusion.",
    addProspect: '+ Ajouter un prospect',
    cancel: '✕ Annuler',
    prospection: 'Prospection',
    negotiation: 'Négociation',
    closed: 'Conclue',
    teamView: 'Vue équipe',
    teamSubtitle: 'Pipeline consolidé de toute votre équipe.',
    noProspect: 'Aucun prospect',
    teamNoProspect: "Votre équipe n'a pas encore de prospects.",
    addProspectTitle: 'Ajouter un prospect',
    addProspectSubtitle: 'Renseignez les informations du nouveau contact.',
    myPipeline: 'Mon pipeline',
    emptyPipeline: 'Pipeline vide',
    emptyPipelineDesc: 'Ajoutez un prospect via "Ajouter un prospect" ou depuis la recherche.',
    company: 'Entreprise',
    city: 'Ville',
    sector: 'Secteur',
    status: 'Statut',
    note: 'Note',
    nextAction: 'Prochaine action',
    placeholderCompanyName: "Nom de l'entreprise",
    placeholderCity: 'Douala, Yaoundé…',
    placeholderSector: 'BTP, Commerce…',
    placeholderNote: 'Observations…',
    placeholderNotes:
      'Saisissez ici toutes les informations utiles à la vente : contexte, décideur, besoins identifiés, objections, historique des échanges…',
    placeholderNextAction: 'Rappel, démo, RDV…',
    addBtn: 'Ajouter au pipeline',
    errorNameRequired: "Le nom de l'entreprise est requis",
    contactInfo: 'Informations de contact',
    phone: 'Téléphone',
    email: 'Email',
    notSpecified: 'Non renseigné',
    notesLabel: 'Notes commerciales',
    notesHint: 'observations, contexte, historique…',
    hasNotes: 'Notes',
    notesSaveBtn: 'Sauvegarder',
    notesSaved: 'Notes sauvegardées',
    notesSaveError: 'Impossible de sauvegarder les notes',
    notesUnsaved: 'Modifications non sauvegardées — cliquez sur Sauvegarder',
    nextFollowUpLabel: 'Prochain suivi',
    assignedBy: 'Assigné par',
    clickForDetails: 'Cliquer pour voir les détails',
    deleteSuccess: 'Prospect supprimé',
    deleteError: 'Suppression impossible',
    assignedTo: 'Assigné à',
    exportPerformances: 'Exporter les performances',
    exportDesc:
      'Téléchargez un fichier Excel avec la synthèse et le détail des prospects par membre.',
    filterMember: 'Filtrer par membre',
    allMembers: 'Tous les membres',
    exportFrom: 'Du',
    exportTo: 'Au',
    exportReset: 'Réinitialiser',
    exportDownload: "Télécharger l'export",
    exportSuccess: 'Export téléchargé avec succès',
    exportError: "Erreur lors de l'export"
  },
  auth: {
    loginTitle: 'Connexion',
    loginSubtitle: 'Bienvenue sur Sales Companion 2.0',
    activateTitle: 'Activer mon accès',
    activateSubtitle: "Renseignez l'ID fourni par votre manager.",
    email: 'Email',
    password: 'Mot de passe',
    newPassword: 'Nouveau mot de passe',
    confirmPassword: 'Confirmer le mot de passe',
    loading: 'Chargement…',
    accessId: "Identifiant d'accès (Access ID)",
    loginBtn: 'Se connecter',
    activateBtn: 'Activer mon compte',
    switchToActivate: "J'ai un accès équipe à activer",
    switchToLogin: 'Retour à la connexion',
    noAccount: 'Pas encore de compte ?',
    createAccount: 'Créer un compte',
    alreadyAccount: 'Déjà un compte ?',
    registerTitle: 'Créer un compte',
    registerSubtitle: 'Rejoignez Sales Companion 2.0',
    fullName: 'Nom complet',
    accountType: 'Type de compte',
    independent: 'Indépendant',
    independentDesc: 'Gérez votre propre pipeline',
    manager: 'Manager',
    managerDesc: 'Gérez une équipe de commerciaux',
    sector: "Secteur d'activité",
    selectSector: 'Sélectionner votre secteur...',
    companyName: "Nom de l'entreprise",
    errorFillAll: 'Veuillez remplir tous les champs.',
    errorPasswordMatch: 'Les mots de passe ne correspondent pas.',
    errorPasswordLength: 'Le mot de passe doit comporter au moins 6 caractères.',
    corporateEmailRequired:
      "L'inscription Manager requiert une adresse email professionnelle d'entreprise (ex: prenom.nom@votre-entreprise.cm). Les adresses grand public (Gmail, Yahoo, Outlook...) ne sont pas autorisées.",
    corporateEmailHint: 'Adresse professionnelle requise (ex: contact@entreprise.cm)',
    noCorporateEmailContactSupport: "Votre entreprise n'a pas de nom de domaine propre ? Contactez le support.",
    activateHeader: 'Activer mon compte',
    activateHeaderSub: 'Créez votre mot de passe pour accéder à Sales Companion 2.0.',
    emailWillBeSent: "Un email de vérification sera envoyé après l'activation.",
    invalidEmail: "L'adresse email est obligatoire pour activer votre compte.",
    invalidEmailFormat: "Format d'adresse email invalide.",
    errorActivation: "Erreur lors de l'activation. Réessayez.",
    verifyingLink: "Vérification du lien d'activation…",
    invalidLink: "Lien d'activation invalide ou expiré.",
    accessImpossible: 'Accès impossible',
    contactManager:
      'Si le problème persiste, contactez votre manager pour obtenir un nouveau lien.',
    unableToLoadAccess: "Impossible de charger les informations d'accès.",
    name: 'Nom',
    company: 'Entreprise',
    yourEmailAddress: 'Votre adresse email',
    professionalEmailLocked: 'Adresse e-mail professionnelle attribuée à votre invitation (non modifiable).',
    emailHint: "Saisissez l'adresse email qui servira d'identifiant pour vous connecter.",
    atLeast6Chars: '✓ Au moins 6 caractères',
    allowedChars: '✓ Lettres, chiffres et caractères spéciaux acceptés',
    activationInProgress: 'Activation en cours...',
    companyAccess: 'Accès Entreprise',
    enterAccessCode: "Veuillez saisir le code d'accès fourni par votre manager.",
    continue: 'Continuer',
    checkEmail: 'Vérifiez votre email',
    verificationEmailSent: 'Un email de vérification a été envoyé à',
    howToProceed: 'Comment procéder :',
    proceedStep1: "Ouvrez l'email de Sales Companion 2.0 dans votre boîte de réception.",
    proceedStep2: 'Cliquez sur le bouton « Vérifier mon adresse email ».',
    proceedStep3: 'Votre compte sera activé automatiquement.',
    noEmail: "Pas d'email ? Vérifiez vos spams ou attendez quelques instants.",
    resend: "Renvoyer l'email",
    pageAutoUpdates: 'Cette page se met à jour automatiquement dès que votre email est vérifié.',
    emailVerified: 'Email vérifié !',
    accountActive: 'Votre compte est maintenant actif. Redirection…'
  },
  field: {
    sector: 'Secteur',
    region: 'Région',
    city: 'Ville',
    telephone: 'Téléphone',
    email: 'Email',
    dirigeant: 'Dirigeant',
    niu: 'NIU',
    rccm: 'RCCM',
    adresse: 'Adresse',
    capital: 'Capital',
    raisonSociale: 'Raison Sociale',
    sigle: 'Sigle',
    formeJuridique: 'Forme Juridique'
  },
  sectors: {
    commerce: 'Commerce',
    btp: 'BTP & Construction',
    industrie: 'Industrie manufacturière',
    agro: 'Agriculture & Agroalimentaire',
    services: 'Services & Conseil',
    transport: 'Transport & Logistique',
    hotellerie: 'Hôtellerie & Restauration',
    sante: 'Santé',
    education: 'Éducation & Formation',
    tech: 'Technologies & Numérique',
    finance: 'Finance & Assurance',
    energie: 'Énergie & Mines'
  },
  profile: {
    title: 'Profil',
    subtitle: "Informations de compte et statistiques d'utilisation.",
    searchesToday: "Recherches aujourd'hui",
    searchesThisMonth: "Recherches ce mois",
    quotaUsed: 'du quota utilisé',
    dailyQuota: 'Quota quotidien',
    monthlyQuota: 'Quota mensuel',
    resetDaily: 'Réinitialisé chaque jour',
    resetMonthly: 'Réinitialisé chaque mois',
    status: 'Statut',
    active: 'Actif',
    inactive: 'Inactif',
    dailyQuotaLabel: 'Quota journalier',
    monthlyQuotaLabel: 'Quota mensuel',
    loading: 'Chargement…',
    defaultName: 'Utilisateur',
    companyLabel: 'Entreprise',
    sectorLabel: "Secteur d'activité",
    regionLabel: 'Région principale',
    noCompany: 'Non renseignée',
    noSector: 'Non renseigné',
    editProfileBtn: 'Modifier mes informations',
    editProfileTitle: 'Modifier le profil professionnel',
    fullNameLabel: 'Nom complet',
    phoneLabel: 'Numéro de téléphone',
    saveBtn: 'Enregistrer les modifications',
    cancelBtn: 'Annuler',
    profileUpdatedToast: 'Profil mis à jour avec succès !',
    profileUpdateErrorToast: 'Erreur lors de la mise à jour du profil.',
    requestChangeViaSupportBtn: 'Demander une modification au support',
    managerProfileLockedNotice:
      "Pour des raisons de gouvernance et de sécurité d'entreprise, les informations du compte Manager sont modifiées sur validation du support.",
    memberProfileLockedNotice:
      'Vos informations de profil sont gérées et synchronisées par votre organisation.',
    supportSubjectProfileChange: "Demande de modification des informations d'entreprise",
    roles: {
      admin: 'Administrateur',
      manager: 'Manager',
      member: 'Commercial',
      independent: 'Indépendant',
      support_agent: 'Agent Support'
    }
  },
  settings: {
    title: 'Paramètres',
    subtitle: "Préférences utilisateur et informations d'abonnement.",
    currentSubscription: 'Abonnement actuel',
    planLabel: 'Plan',
    unlimitedSearches: 'Recherches illimitées',
    searchesPerDay: 'recherches / jour',
    searchesPerDay1000: '50 recherches / jour',
    upgradeBtn: 'Passer à un plan supérieur',
    upgradeAdminHint: 'Contactez votre Admin pour upgrade',
    account: 'Compte',
    noAdvancedParams: 'Aucun paramètre avancé disponible',
    managerUpgradeHint: "Passez au plan Manager pour accéder aux paramètres d'équipe.",
    plans: {
      free: 'Gratuit',
      starter: 'Starter',
      pro: 'Pro',
      enterprise: 'Entreprise'
    },
    appearanceTitle: 'Apparence',
    appearanceSubtitle: 'Personnalisez le style visuel de votre interface',
    themeLinkedinDesc: 'Épuré et professionnel. Bleu LinkedIn, arrondis discrets.',
    themeFirebaseDesc: 'Amber & Navy. Dark-first, moderne et technique.',
    activeBadge: 'ACTIF',
    activeThemeLabel: 'Thème actif :',
    switchThemeBtn: 'Basculer →',
    securityTitle: 'Sécurité & Compte',
    securitySubtitle: 'Gérez vos informations de connexion et de sécurité',
    emailTitle: 'Adresse e-mail',
    currentEmailLabel: 'Votre adresse e-mail actuelle est :',
    memberEmailLocked: "En tant que membre d'équipe, votre adresse e-mail professionnelle est gérée par votre organisation et ne peut pas être modifiée.",
    supportEmailLocked: "En tant qu'agent de support, votre adresse e-mail professionnelle est gérée par votre organisation et ne peut pas être modifiée.",
    googleEmailNote: "Votre compte est associé à Google. Les modifications d'adresse e-mail doivent être effectuées depuis votre compte Google.",
    newEmailPlaceholder: 'Nouvelle adresse e-mail',
    updateBtn: 'Mettre à jour',
    emailHint: 'Un e-mail de confirmation sera envoyé à la nouvelle adresse pour valider le changement.',
    emailUpdateSuccess: 'Un e-mail de vérification a été envoyé. Veuillez cliquer sur le lien pour confirmer.',
    emailUpdateError: "Erreur lors de la mise à jour de l'e-mail",
    passwordTitle: 'Mot de passe',
    googlePasswordNote: 'Votre compte est associé à Google. Votre mot de passe est géré de manière sécurisée par Google.',
    sendResetBtn: "Envoyer l'email de réinitialisation",
    passwordHint: 'Nous vous enverrons un lien de réinitialisation sécurisé par e-mail.',
    passwordResetSuccess: 'Un e-mail de réinitialisation de mot de passe a été envoyé à votre adresse.',
    passwordResetError: "Erreur lors de l'envoi de l'e-mail",
    themeActivatedToast: 'Thème activé',
    features: {
      basicSearch: 'Recherche basique',
      personalPipeline: 'Pipeline personnel',
      fiftySearches: '10 recherches/jour',
      advancedSearch: 'Recherche avancée',
      twoHundredSearches: '20 recherches/jour',
      excelExport: 'Export Excel',
      allStarter: 'Tout Starter',
      teamManagement: 'Gestion équipe',
      oneThousandSearches: '50 recherches/jour',
      aiAssistant: 'Companion IA',
      prioritySupport: 'Support prioritaire',
      allPro: 'Tout Pro',
      unlimited: 'Illimité',
      pipelineUnlimited: 'Pipeline illimité',
      dedicatedDeployment: 'Déploiement dédié',
      dedicatedSupport: 'Support dédié'
    }
  },
  regions: {
    adamaoua: 'Adamaoua',
    centre: 'Centre',
    est: 'Est',
    extremeNord: 'Extrême-Nord',
    littoral: 'Littoral',
    nord: 'Nord',
    nordOuest: 'Nord-Ouest',
    ouest: 'Ouest',
    sud: 'Sud',
    sudOuest: 'Sud-Ouest'
  },
  team: {
    title: "Gestion de l'équipe",
    subtitle: 'Gérez les membres de votre équipe, leurs accès et vos prospects importés.',
    tabTeam: 'Mon équipe',
    tabImports: 'Mes prospects importés',
    prospectsSelected: "prospect(s) sélectionné(s) depuis la liste d'imports",
    clearSelection: 'Effacer la sélection',
    importTitle: '📥 Importer une base de prospects',
    importDesc:
      'Importez vos prospects depuis un fichier CSV. La base sera accessible uniquement à vous et votre équipe.',
    myProspects: '📋 Mes prospects',
    myProspectsDesc:
      'Cochez des prospects et cliquez sur "Assigner la sélection" pour les retrouver dans le formulaire d\'assignation.',
    goToForm: 'Aller au formulaire ↗',
    errorFormat: 'Format non supporté. Utilisez un fichier texte (.csv, .txt, .tsv, .xlsx, etc.)',
    errorNoValid: 'Aucun prospect valide trouvé. Vérifiez le format du fichier.',
    errorMaxRows: 'lignes détectées. Maximum 3000 par import.',
    errorRead: "Impossible de lire le fichier. Vérifiez qu'il est bien encodé en UTF-8.",
    errorServer: 'Erreur serveur',
    importSuccess: 'prospect(s) importé(s) avec succès',
    importError: "Erreur lors de l'import",
    dragDrop: 'Glissez votre fichier CSV ici',
    orClick: 'ou cliquez pour sélectionner',
    prospectsDetected: 'prospect(s) détecté(s)',
    supportedFormats: 'Formats supportés :',
    autoSeparators: 'Séparateurs détectés automatiquement :',
    separatorsList: 'virgule, point-virgule, tabulation, pipe (|)',
    flexibleColumns:
      "Les noms de colonnes sont flexibles (français ou anglais). Le système s'adapte automatiquement.",
    preview: 'Aperçu',
    importing: '⏳ Import en cours…',
    importBtn: '⬆ Importer',
    cancel: 'Annuler',
    searchProspect: 'Rechercher un prospect…',
    allMembers: 'Tous les membres',
    unassigned: 'Non assignés',
    notAssigned: 'Non assigné',
    refresh: 'Actualiser',
    loading: 'Chargement…',
    prospectsOn: 'prospect(s) sur',
    selected: 'sélectionné(s)',
    assignSelection: '↗ Assigner la sélection',
    noProspectImported: 'Aucun prospect importé. Utilisez le bouton "Importer" ci-dessus.',
    noProspectMatch: 'Aucun prospect correspond à votre recherche.',
    selectAll: 'Tout sélectionner',
    statusNew: 'Nouveau',
    statusContacted: 'Contacté',
    statusInProgress: 'En cours',
    statusConverted: 'Converti',
    statusLost: 'Perdu',
    createAccess: 'Créer un accès collaborateur',
    createAccessDesc: 'Générez un identifiant unique pour inviter un membre dans votre équipe.',
    quotaLabel: 'Quota de collaborateurs :',
    quotaReached: 'Limite atteinte. Passez au plan supérieur pour ajouter plus de membres.',
    firstname: 'Prénom',
    lastname: 'Nom',
    companyOptional: 'Entreprise (Optionnel)',
    accessIdPreview: "Aperçu de l'Identifiant (Access ID) :",
    creating: 'Création...',
    generateAccess: "Générer l'accès",
    generatedAccesses: 'Accès générés',
    totalAccesses: 'accès au total',
    noAccess: 'Aucun accès',
    noAccessDesc: "Vous n'avez pas encore généré d'accès pour votre équipe.",
    active: 'Actif',
    revoked: 'Révoqué',
    pendingEmail: '⏳ Email en attente',
    pending: 'En attente',
    copyAccessId: "Copier l'identifiant d'accès",
    copy: 'Copier',
    revokeAccess: "Révoquer l'accès",
    revoke: 'Révoquer',
    deletePermanently: 'Supprimer définitivement',
    delete: 'Supprimer',
    deleteWarning: 'Son accès et son pipeline seront supprimés.',
    deleting: 'Suppression...',
    confirmDeleteBtn: 'Confirmer la suppression',
    assignProspect: 'Assigner un prospect',
    assignProspectBulk: 'prospect(s) sélectionné(s) depuis la liste',
    assignProspectSingle: "Attribuez un prospect à un membre de l'équipe",
    successCreated: 'assignation(s) créée(s) avec succès',
    selectedProspects: 'Prospects sélectionnés',
    prospect: 'Prospect',
    selectProspect: 'Sélectionner un prospect…',
    noProspectAvailable: 'Aucun prospect disponible',
    teamMember: "Membre de l'équipe",
    selectMember: 'Sélectionner un membre…',
    noActiveMember: 'Aucun membre actif',
    noActiveMemberDesc:
      "Aucun membre actif. Les membres doivent activer leur compte avant d'être assignables.",
    assigning: 'Assignation en cours…',
    assignProspectBtn: 'Assigner le prospect',
    assignSelectedBtn: 'Assigner',
    teamMembers: "Membres de l'équipe",
    activeMembersCount: 'membre(s) actif(s)',
    emptyTeam: "Aucun membre dans l'équipe",
    noMemberAssigned: "Aucun membre n'a été assigné à cette équipe.",
    dailyQuota: 'Quota quotidien',
    inactive: 'Inactif',
    activeAssignments: 'Assignations actives',
    prospectAssigned: 'prospect(s) assigné(s)',
    repairNetworkError: 'Erreur réseau',
    repairPipelines: '🔧 Réparer les pipelines membres',
    repairingPipelines: '⏳ Réparation…',
    repairSuccessSync: 'membre(s) synchronisé(s) (pipeline visible)',
    repairSuccessName: 'nom(s) corrigé(s)',
    repairSkipped: '✓ Tous les pipelines sont déjà synchronisés (ignorés)',
    repairErrors: 'erreur(s)',
    noAssignmentCreated: "Aucune assignation n'a encore été créée.",
    date: 'Date',
    prospectAssignedSub: 'Prospect assigné',
    deactivate: 'Désactiver',
    activate: 'Activer',
    remove: 'Supprimer',
    supportTitle: '📁 Fichier Clients',
    supportSubtitle: 'Importez votre base de données clients. Les données apparaîtront directement dans Mes Clients CRM.',
    supportImportTitle: '📥 Importer une base de données clients',
    supportImportSubtitlePrefix: 'Glissez un fichier CSV, Excel ou texte. Les clients importés seront immédiatement visibles dans',
    supportImportedTitle: '📋 Clients importés',
    supportImportedSubtitlePrefix: 'Aperçu de votre fichier importé. Ces entrées sont visibles dans',
    accountNotConfigured: "⚠️ Compte non configuré. Contactez l'administrateur.",
    exampleColumns: 'Nom ; Téléphone | Email , Ville'
  },
  support: {
    title: 'Support',
    subtitle: 'Suivez vos conversations avec notre équipe',
    cancel: 'Annuler',
    newConversation: 'Nouvelle conversation',
    subjectLabel: 'Sujet de votre demande',
    subjectPlaceholder: "Ex: Problème d'accès, Question sur mon plan…",
    start: 'Démarrer',
    errorLoad: 'Impossible de charger les conversations. Vérifiez votre connexion.',
    conversationsList: 'Conversations',
    noConversation: 'Aucune conversation',
    startNew: 'Démarrez une nouvelle conversation avec le support.',
    selectOrStart: 'Sélectionnez une conversation ou démarrez-en une nouvelle.',
    startBySending: 'Démarrez la conversation en envoyant un message ci-dessous.',
    me: 'Moi',
    supportTeam: '🎧 Support',
    resolvedThread: '✓ Conversation résolue.',
    needMoreHelp: "Besoin d'aide supplémentaire ?",
    typeMessage: 'Écrivez votre message… (Entrée pour envoyer)',
    confirmDelete: 'Voulez-vous vraiment supprimer cette conversation ?',
    confirmDeleteMessage: 'Voulez-vous vraiment supprimer ce message ?',
    deleteMessage: 'Supprimer ce message',
    errorDeleteMessage: 'Une erreur est survenue lors de la suppression du message.',
    errorDelete: 'Une erreur est survenue lors de la suppression.',
    deleteTitle: 'Supprimer la conversation',
    statusOpen: 'En cours',
    statusResolved: 'Résolu',
    statusClosed: 'Fermé',
    publicTitle: 'Assistance & Validation de Compte',
    publicSubtitle: "Soumettez votre demande directement à l'équipe support Sales Companion 2.0.",
    corporateDomainBannerTitle: "Création de compte Manager sans domaine d'entreprise personnalisé",
    corporateDomainBannerDesc: "Par mesure de sécurité, les comptes Manager requièrent habituellement un domaine email d'entreprise. Si vous utilisez une messagerie sans nom de domaine personnalisé, notre équipe d'administration peut valider manuellement votre profil pour vous donner l'accès.",
    nameLabel: 'Nom complet',
    emailLabel: 'Email de contact',
    companyLabel: "Nom de l'entreprise",
    phoneLabel: 'Téléphone / WhatsApp',
    requestTypeLabel: 'Type de demande',
    typeCorporateDomain: 'Dérogation domaine entreprise (Manager)',
    typeAccessIssue: "Problème d'accès ou d'inscription",
    typeCommercial: 'Demande commerciale & Démo',
    typeOther: 'Autre demande',
    sectorLabel: "Secteur d'activité",
    selectSector: 'Sélectionnez votre secteur…',
    messageLabel: 'Message / Détails',
    messagePlaceholder: 'Expliquez brièvement votre demande ou votre activité…',
    defaultCorporateMsg: "Bonjour,\n\nNotre entreprise n'utilise pas de domaine email personnalisé (ex: @notre-entreprise.cm) mais utilise des emails standards. Nous souhaitons créer un compte Manager pour piloter notre équipe commerciale.\n\nMerci de bien vouloir étudier notre demande d'activation.",
    submitBtn: 'Envoyer ma demande au support',
    submitting: 'Envoi en cours…',
    successTitle: 'Demande transmise avec succès !',
    successDesc: "Votre requête a bien été enregistrée et transmise à notre équipe administrative. Un responsable va examiner les informations de votre entreprise et vous contacter sous peu par email.",
    recapTitle: 'Récapitulatif de votre demande',
    backToRegister: "Retour à l'inscription",
    backToLogin: 'Connexion',
    errorNameRequired: 'Veuillez saisir votre nom complet.',
    errorEmailRequired: 'Veuillez saisir une adresse email valide.',
    errorCompanyRequired: "Veuillez renseigner le nom de l'entreprise.",
    errorMessageRequired: 'Veuillez décrire votre demande.'
  },
  saved: {
    title: 'Mes favoris',
    subtitle: 'Retrouvez vos entreprises et recherches enregistrées.',
    mySearches: 'Mes recherches',
    searchesSaved: 'recherche(s) sauvegardée(s)',
    errorLoad: 'Impossible de charger les recherches sauvegardées.',
    noSearch: 'Aucune recherche sauvegardée',
    noSearchDesc:
      'Utilisez le bouton "Sauvegarder" lors d\'une recherche pour retrouver vos filtres ici.',
    restoreSuccess: 'Recherche restaurée',
    deleteSuccess: 'Recherche supprimée',
    deleteError: 'Suppression impossible',
    unknownError: 'Erreur inconnue',
    myCompanies: 'Mes entreprises sauvegardées',
    errorLoadCompanies: 'Erreur lors du chargement des entreprises',
    noCompany: 'Aucune entreprise sauvegardée',
    noCompanyDesc: "Vous n'avez pas encore sauvegardé d'entreprises individuelles.",
    searchCompanies: 'Rechercher des entreprises',
    startSearch: 'Lancer une recherche',
    deleteCompanySuccess: 'Entreprise supprimée de vos favoris',
    deleteCompanyError: 'Erreur de suppression'
  },
  admin: {
    dashboardTitle: 'Dashboard admin',
    dashboardSubtitle: 'Vue consolidée des statistiques de la plateforme.',
    users: 'Utilisateurs',
    companies: 'Entreprises',
    pipelineItems: 'Items pipeline',
    searchesToday: "Recherches aujourd'hui",
    activeThisWeek: 'actifs cette semaine',
    thisWeek: 'cette semaine',
    inDatabase: 'Dans la base de données',
    allReps: 'Tous les commerciaux',
    acrossPlatform: "Sur l'ensemble de la plateforme",
    usersByRole: '📊 Utilisateurs par rôle',
    plansDistribution: '💎 Répartition des plans',
    recentActivity: '📈 Activité récente',
    newUsers: 'Nouveaux utilisateurs',
    searches: 'Recherches',
    activationRate: "Taux d'activation",
    companiesPerUser: 'Entreprises / Utilisateur',
    activePct: '% actifs',
    avg: 'moy.',
    today: "aujourd'hui",
    loadingStats: 'Impossible de charger les statistiques admin.',

    logsTitle: "Journal d'activité",
    logsSubtitle: '20 dernières recherches effectuées sur la plateforme.',
    recentSearches: 'Recherches récentes',
    last20: '20 dernières',
    refresh: '↻ Actualiser',
    loadingLogs: 'Impossible de charger les logs.',
    user: 'Utilisateur',
    email: 'Email',
    query: 'Requête',
    resultsCount: 'Résultats',
    plan: 'Plan',
    date: 'Date',
    noActivity: "Aucune activité enregistrée pour l'instant.",

    usersTitle: 'Gestion des utilisateurs',
    usersSubtitle: 'Gérez les rôles, les quotas et les accès.',
    allUsers: 'Tous les utilisateurs',
    searchUsers: 'Rechercher par nom ou email...',
    filterRole: 'Filtrer par rôle...',
    filterPlan: 'Filtrer par plan...',
    members: 'Membres',
    managers: 'Managers',
    indep: 'Indépendants',
    admins: 'Administrateurs',
    role: 'Rôle',
    status: 'Statut',
    quota: 'Quota / J',
    created: 'Création',
    lastLogin: 'Dernière co.',
    actions: 'Actions',
    noUserMatch: 'Aucun utilisateur ne correspond.',
    saving: 'Enregistrement...',

    companiesTitle: 'Gestion des entreprises',
    companiesSubtitle: 'Parcourez et modifiez les entreprises de la base de données.',
    allCompanies: 'Toutes les entreprises',
    searchCompanies: 'Rechercher une entreprise...',
    addCompany: '+ Ajouter une entreprise',
    companyName: 'Raison Sociale',
    city: 'Ville',
    sector: 'Secteur',
    phone: 'Téléphone',
    noCompanyMatch: 'Aucune entreprise ne correspond à cette recherche.',
    exportCsv: 'Exporter (CSV)',
    companyStats: 'Statistiques des entreprises',
    totalCompanies: 'Total',
    bySector: 'PAR SECTEUR',
    byRegion: 'PAR RÉGION',
    notSpecified: 'Non spécifié',
    declaredActivity: 'Activité déclarée',
    selectToDelete: 'Sélectionnez des entreprises à supprimer.',
    clearSelection: 'Vider la liste',
    deleteSelection: 'Supprimer la sélection',
    clearHistory: "Effacer l'historique",

    importsTitle: 'Import de données',
    importsSubtitle: 'Historique des imports en masse réalisés sur la plateforme.',
    allImports: 'Tous les imports',
    newImport: '+ Nouvel import',
    filename: 'Fichier',
    count: 'Lignes',
    importResultsCount: 'Résultats',
    importStatus: 'Statut',
    importFileDate: 'Date',
    importRefresh: 'Actualiser',
    importedBy: 'Importé par',
    importDate: "Date d'import",
    noImports: 'Aucun import enregistré.',
    viewDetails: 'Voir les détails',
    dragFile: 'Glissez votre fichier ici',
    orBrowse: 'ou cliquez pour parcourir',
    chooseFile: 'Choisir un fichier',
    processing: 'Traitement de',
    importSuccess: 'Import réussi',
    rowsRead: 'Lignes lues',
    newRows: 'Nouvelles',
    updatedRows: 'Mises à jour',
    skippedRows: 'Doublons',
    errorRows: 'Erreurs',
    mappedColumns: 'Colonnes mappées',
    importAnother: 'Importer un autre fichier',
    retry: 'Réessayer',
    autoColumns: 'Colonnes reconnues automatiquement :',
    autoColumnsDesc1: "— Nom de l'entreprise (obligatoire)",
    autoColumnsDesc2: "— Numéro d'identification unique (optionnel, clé dédoublonnage)",
    autoColumnsDesc3: '— Secteur auto-détecté',
    autoColumnsDesc4: '— Région / Ville auto-détectées',
    autoColumnsDesc5: '+ SIGLE, TELEPHONE, EMAIL, DIRIGEANT, RCCM (si disponibles)',
    errorFormat: 'Format non pris en charge. Utilisez .xlsx, .xls ou .csv',
    errorServer: 'Erreur serveur',
    errorNetwork: 'Erreur réseau',
    errorDelete: 'Erreur lors de la suppression',
    confirmClearHistory: "Voulez-vous vraiment effacer tout l'historique des imports ? Cette action ne supprime pas les entreprises importées.",
    statusPending: 'En attente',
    statusProcessing: 'En cours',
    statusCompleted: 'Terminé',
    statusFailed: 'Échoué',
    successRate: 'réussi',

    configTitle: 'Configuration',
    configSubtitle: 'Paramètres globaux de la plateforme.',
    globalSettings: 'Paramètres globaux',
    maintenanceMode: 'Mode maintenance',
    maintenanceDesc: "Restreint l'accès à la plateforme (sauf Admins).",
    saveConfig: 'Enregistrer la configuration',
    configSaved: 'Configuration enregistrée',
    apiKeyGroq: 'Clé API Groq',
    apiKeyGroqLabel: "Clé API Groq (pour l'assistant chat)",
    apiKeyGroqPlaceholder: 'gsk_...',
    apiKeyGroqHelp1: 'Obtenez votre clé gratuite sur ',
    apiKeyGroqHelp2: ". Utilisée uniquement pour l'assistant chat.",
    saveApiKey: 'Enregistrer la clé',
    adminSecurity: 'Sécurité admin',
    newPassword: 'Nouveau mot de passe',
    newPasswordPlaceholder: 'Minimum 6 caractères',
    newPasswordHelp: 'Changez le mot de passe administrateur par défaut.',
    changePassword: 'Changer le mot de passe',
    pricingPlans: 'Plans tarifaires',
    searchesPerDay: 'Recherches / jour',
    recommendedMonthlyPrice: 'Prix mensuel recommandé',
    target: 'Cible',
    targetFree: 'Essai / découverte',
    targetStarter: 'Commerciaux indépendants',
    targetPro: 'Équipes commerciales, PME',
    targetEnterprise: 'Grandes entreprises, cabinets',
    unlimited: 'Illimité',

    supportTitle: 'Centre de Support Admin',
    supportSubtitle: 'Gérez et répondez aux tickets utilisateurs.',
    allTickets: 'Tous les tickets',
    openTickets: 'Ouverts',
    closedTickets: 'Fermés',
    noTickets: 'Aucun ticket.',
    ticketSubject: 'Sujet',
    ticketFrom: 'De',
    ticketStatus: 'Statut',
    ticketDate: 'Date',
    resolve: 'Marquer comme résolu',
    reply: 'Répondre',
    sendReply: 'Envoyer',
    searchTickets: 'Rechercher par sujet, nom, email, ID...',
    displayedTickets: 'affichés',
    clickToRead: '← Cliquez sur un ticket pour lire le message',
    deletePermanently: 'Supprimer définitivement',
    confirmDeleteTicket:
      'Voulez-vous vraiment supprimer définitivement ce ticket et tous ses messages ?',
    errorLoading: 'Erreur lors du chargement',
    errorOpening: "Erreur lors de l'ouverture du ticket",
    errorSending: "Erreur lors de l'envoi",
    errorDeleting: 'Erreur lors de la suppression du ticket.',
    errorResolving: 'Erreur lors de la résolution',
    userProfile: 'Utilisateur',
    adminReply: 'Support',
    yourReply: 'Votre réponse… (Ctrl+Entrée pour envoyer)',
    noMessageInTicket: 'Aucun message dans ce ticket.',
    identifier: 'Identifiant (UID)',
    clickToCopy: "Cliquer pour copier l'ID",

    paymentsTitle: 'Validation des paiements',
    paymentsSubtitle: 'Gérez et validez les demandes de paiement manuel de plan.',
    allPayments: 'Toutes les demandes de paiement',
    noPayments: 'Aucune demande en attente.',
    paymentValidated: 'Paiement validé avec succès.',
    paymentRejected: 'Paiement rejeté.',
    paymentError: 'Une erreur est survenue.',
    paymentDetail: 'Détails du paiement',
    paymentActions: 'Actions',
    paymentStatus: 'Statut',
    paymentUser: 'Utilisateur',
    paymentPlan: 'Plan demandé',
    paymentAmount: 'Montant',
    paymentOperator: 'Opérateur',
    paymentTransaction: 'ID Transaction',
    paymentDate: 'Date de demande',
    paymentConfirmValidate: 'Voulez-vous vraiment valider ce paiement ? Cela activera immédiatement le plan correspondant pour l\'utilisateur.',
    paymentConfirmReject: 'Voulez-vous vraiment rejeter ce paiement ?',
    validateBtn: 'Valider',
    rejectBtn: 'Rejeter',
    paymentStatusPending: 'En attente',
    paymentStatusDone: 'Validé',
    paymentStatusFailed: 'Échoué'
  },
  crm: {
    title: 'Mes Clients CRM',
    subtitleSingular: 'client · Cliquez pour gérer',
    subtitlePlural: 'clients · Cliquez pour gérer',
    headerClients: 'clients',
    headerActive: 'actifs',
    headerToFollowUp: 'à relancer',
    addClient: 'Ajouter un client',
    searchPlaceholder: 'Rechercher un client, ville, secteur, téléphone…',
    totalClients: 'Total clients',
    displayed: 'Affichés',
    loading: 'Chargement des clients…',
    noResult: 'Aucun résultat',
    noClients: 'Aucun client pour le moment',
    noResultDesc: 'Essayez un autre terme de recherche',
    noClientsDesc: "Ajoutez votre premier client ou importez depuis le pipeline.",
    colCompany: 'Entreprise',
    colCity: 'Ville',
    colSector: 'Secteur',
    colPhone: 'Téléphone',
    colAction: 'Action',
    viewDetails: 'Consulter',
    open: 'Ouvert',
    manage: 'Gérer',
    filtersAllClients: 'Tous',
    filtersActive: 'Actifs',
    filtersToFollowUp: 'À relancer',
    filterAllStatuses: 'Tous les statuts',
    filterAllSectors: 'Tous les secteurs',
    filterAllCities: 'Toutes les villes',
    sortLastActivity: 'Dernière activité',
    sortNextAction: 'Prochaine action',
    sortName: 'Nom',
    sortStatus: 'Statut',
    statusNew: 'Nouveau',
    statusToContact: 'À contacter',
    statusContacted: 'Contacté',
    statusInDiscussion: 'En discussion',
    statusProposalSent: 'Proposition envoyée',
    statusWon: 'Client',
    statusLost: 'Perdu',
    statusImported: 'Importé',
    table: {
      colCompany: 'Entreprise',
      colContact: 'Contact',
      colStatus: 'Statut',
      colLastActivity: 'Dernière activité',
      colNextAction: 'Prochaine action',
      colActions: 'Actions',
      today: "Aujourd'hui",
      yesterday: 'Hier',
      noActivity: 'Aucune activité',
      nextActionPlaceholder: 'Ex: Rappeler vendredi…',
      addNextAction: 'Ajouter une action',
      view: 'Voir la fiche',
      call: 'Appeler',
      delete: 'Supprimer',
      showing: 'Affichage'
    },
    drawer: {
      call: 'Appeler',
      newTicket: 'Nouveau ticket',
      tabCalls: 'Appels',
      tabSupport: 'SAV',
      tabHistory: 'Historique',
      loading: 'Chargement…',
      noCalls: 'Aucun appel enregistré',
      noCallsSub: "Cliquez sur 'Appeler' pour initier un appel",
      noTickets: 'Aucun ticket ouvert',
      noTicketsSub: 'Créez un ticket pour suivre une réclamation',
      noHistory: 'Aucun historique',
      noHistorySub: 'Les appels et tickets apparaîtront ici',
      openTicketBtn: 'Ouvrir un nouveau ticket SAV',
      callEvent: 'Appel',
      ticketEvent: 'Ticket',
      callModalTitle: 'Appel avec',
      qualifyCall: "Qualifiez l'appel",
      callResultLabel: "Résultat de l'appel",
      notesLabel: 'Notes / Compte-rendu',
      notesPlaceholder: "Résumez l'échange en quelques mots…",
      saving: 'Enregistrement…',
      saveCall: "Enregistrer l'appel",
      newTicketModalTitle: 'Nouveau Ticket SAV',
      forCompany: 'Pour',
      subjectLabel: 'Sujet *',
      subjectPlaceholder: 'Ex: Problème de facturation',
      descLabel: 'Description',
      descPlaceholder: 'Décrivez le problème en détail…',
      priorityLabel: 'Priorité',
      creating: 'Création…',
      createTicket: 'Créer le ticket',
      statusConnected: 'Décroché',
      statusNoAnswer: 'Non joint',
      statusBusy: 'Occupé',
      statusVoicemail: 'Répondeur',
      statusFailed: 'Échec',
      priorityLow: 'Basse',
      priorityMedium: 'Moyenne',
      priorityHigh: 'Haute',
      priorityUrgent: 'Urgente',
      ticketOpen: 'Ouvert',
      ticketInProgress: 'En cours',
      ticketResolved: 'Résolu',
      ticketClosed: 'Fermé'
    }
  },
  offline: {
    banner: 'Vous êtes hors ligne — Les données en cache restent accessibles',
    reconnected: 'Connexion rétablie — Synchronisation en cours…',
    searchUnavailable: 'Recherche indisponible hors ligne',
    cachedResults: 'Résultats en cache (mode hors ligne)',
    aiUnavailable: "L'assistant IA nécessite une connexion internet",
    lastSync: 'Dernière synchronisation :',
    pendingChanges: 'modifications en attente de synchronisation'
  },
  common: {
    next: 'Suivant',
    previous: 'Précédent',
    edit: 'Modifier',
    cancel: 'Annuler',
    reset: 'Réinitialiser',
    retry: 'Réessayer'
  }
}

