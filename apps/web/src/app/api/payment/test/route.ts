...
export async function GET(request: NextRequest) {
  ...
  const config = {
    ...
    CAMPAY_PERMANENT_TOKEN: PERMANENT_TOKEN
      ? `SET (longueur: ${PERMANENT_TOKEN.length})`
      : '❌ NON DÉFINI',
  ...
  if (PERMANENT_TOKEN) {
    ...
    if (APP_USERNAME) {
      ...
      results['C_username_plus_permanentToken_as_password'] =
        '⏭ Ignoré (CAMPAY_APP_USERNAME non défini)'
    }
  }
  ...
  const conclusion = Object.entries(results).find(
    ([, v]) =>
      typeof v === 'object' &&
      v !== null &&
      'status' in v &&
      (v as Record<string, unknown>).status === 200
  )
