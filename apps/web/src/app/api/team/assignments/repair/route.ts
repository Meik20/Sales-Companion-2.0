...
export async function POST(request: NextRequest) {
  try {
    ...
    async function resolveUid(email: string): Promise<string | null> {
      ...
      try {
        const taSnap = await adminDb.collection('team_accesses').where('accessId', '==', email).limit(1).get()
        if (!taSnap.empty) {
    ...
    for (const doc of allPipelineSnap.docs) {
      ...
      if (isEmailLike(currentUserId)) {
        ...
        if (uid) { updates.userId = uid; updates.assignedTo = uid; uidFixed++ }
      }
    ...
    const teamSnap = await adminDb.collection('team_assignments').where('managerUid', '==', managerUid).get()
    for (const aDoc of teamSnap.docs) {
      ...
      if (isEmailLike(rawMemberId)) {
        ...
        if (uid) {
          realMemberId = uid
          await aDoc.ref.update({ memberId: uid, updatedAt: FieldValue.serverTimestamp() }).catch(() => {})
        }
      }
      ...
      if (!pipelineEntryId && pipelineItemId) {
        ...
        if (prospectData) {
          ...
          await newRef.set({
            ...
            companyName, name: companyName, status: 'prospection', sourceProspectId: pipelineItemId,
    ...
    const managerPipelineSnap = await adminDb.collection('pipeline').where('userId', '==', managerUid).get()
    const managerPipelineIds = new Set(managerPipelineSnap.docs.map(d => d.id))
    
    // Refresh snap for cleanup
    const freshTeamSnap = await adminDb.collection('team_assignments').where('managerUid', '==', managerUid).get()
    ...
    for (const a of sortedAssignments) {
      ...
      if (!inManagerPipeline && !existsInSources) {
        ...
        if (a.data.pipelineEntryId) {
          await adminDb.collection('pipeline').doc(a.data.pipelineEntryId).delete().catch(() => {})
        }
        deletedStale++
      }
    }
    ...
    return NextResponse.json({ 
      uidFixed, nameFixed, skipped, deletedDupes, deletedStale,
